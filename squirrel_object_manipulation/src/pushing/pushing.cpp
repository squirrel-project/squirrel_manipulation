...
#include <squirrel_object_manipulation/eband_local_planner/eband_local_planner.h>
#include <squirrel_object_manipulation/eband_local_planner/eband_visualization.h>
    ...
    objectLost_(false)
{
    ...
    private_nh.param("global_costmap/global_frame", global_frame_, std::string(global_frame_));
...
void PushAction::executePush(const squirrel_manipulation_msgs::PushGoalConstPtr &goal) {
    ...
    //    eband_local_planner::EBandPlanner *bplan= new eband_local_planner::EBandPlanner();
    //    bplan->initialize("bubble", planner_costmap_ros_);

    //    boost::shared_ptr<eband_local_planner::EBandVisualization>  vis;
    //    bplan->setVisualization(vis);

    //    bplan->setPlan(pushing_path_.poses);
    ...
    //if pushing did not result wit success
    abortPush();
    return;
...
bool PushAction::stopTracking() {
    ...
    if (ros::service::call("/squirrel_stop_object_tracking", srvStopTrack)){
        trackingStart_ = false;
        return true;
    }
    return false;
